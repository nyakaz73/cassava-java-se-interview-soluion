package com.econetwireless.epay.business.aspects;
...
@Component
@Aspect
public class RequestBusinessInterceptor {
    private static final Logger LOGGER = LoggerFactory.getLogger(RequestBusinessInterceptor.class);
    @Autowired  private PartnerCodeValidator partnerCodeValidator;
    private RequestPartnerDao requestPartnerDao;

    @Autowired private ReportingService reportingService;

    public RequestBusinessInterceptor(RequestPartnerDao requestPartnerDao){
        this.requestPartnerDao = requestPartnerDao;
    }
    @Around("execution(* com.econetwireless.epay.business.rest.resources.EpayBusinessResource.creditAirtime(..)) && args(airtimeTopupRequest)")
    public AirtimeTopupResponse creditAirtime(final ProceedingJoinPoint joinPoint, final AirtimeTopupRequest airtimeTopupRequest) {
        ...
        try{
            LOGGER.info("IN BUSINESSS ECredit Airtime :: Partner Code : {}, Mobile Number : {}",
    ...
    @Around("execution(* com.econetwireless.epay.business.rest.resources.EpayBusinessResource.enquireAirtimeBalance(..)) && args(partnerCode, msisdn)")
    public AirtimeBalanceResponse enquireAirtimeBalance(final ProceedingJoinPoint joinPoint, final String partnerCode, final String msisdn) {
        ...
        try {
            LOGGER.info("IN Business Airtime Balance :: Partner Code : {}, Mobile Number : {}",
                    partnerCode, msisdn);
    ...
    @Around("execution(* com.econetwireless.epay.business.rest.resources.EpayBusinessResource.validatePartnerCode(..)) && args(partnerCode)")
    public PartnerResponse validatePartnerCodeData(final ProceedingJoinPoint joinPoint,String partnerCode) throws Throwable {
        PartnerResponse partnerResponse = new PartnerResponse();
        RequestPartner requestPartner = requestPartnerDao.findByCode(partnerCode);
        LOGGER.info("Validate Partner Response");
//        final Object response = joinPoint.proceed();
//        partnerResponse = (PartnerResponse) response;
//        LOGGER.info(partnerResponse.toString());
        partnerResponse.setName(requestPartner.getName());
        partnerResponse.setDescription(requestPartner.getDescription());
        partnerResponse.setCode(requestPartner.getCode());
        LOGGER.info(partnerResponse.toString());
        return  partnerResponse;
    }

    @Around("execution(* com.econetwireless.epay.business.rest.resources.EpayBusinessResource.getPartnerTransactions(..)) && args(partnerCode)")
    ...
    private static AirtimeTopupResponse checkingMissingFields(final AirtimeTopupRequest airtimeTopupRequest) {
        ...
        if(builder.length() > 0) {
            LOGGER.info("REFENCE IN BUSINES NOT FOUND");
